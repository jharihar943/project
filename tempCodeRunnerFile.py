        ...
        if language == 'python':
            ...
            try:
                ...
                with tempfile.TemporaryDirectory() as temp_dir:
                    # Extract class name from code (with or without 'public' keyword)
                    import re
            ...
            try:
                with tempfile.TemporaryDirectory() as temp_dir:
                    ...
                    # Compile TypeScript - use tsc.cmd on Windows
                    tsc_cmd = 'tsc.cmd' if sys.platform == 'win32' else 'tsc'
                    compile_process = subprocess.run(
                        [tsc_cmd, ts_file, '--outFile', js_file],
                        capture_output=True,
                        text=True,
                        timeout=30,
                ...
                result['status'] = 'error'
                result['errors'].append(' TypeScript/Node.js not found! Install Node.js and run: npm install -g typescript')
            except Exception as e:
                result['status'] = 'error'
